// Vercel Serverless Function: POST /api/send-email-hsm
// Sends a professionally letter-headed HTML email branded for The IIE
// School of Hospitality and Service Management (IIEHSM, an Emeris brand).
//
// Required env vars:
//   SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASS
//   ADMIN_AUTH_TOKEN
// Optional:
//   MAIL_FROM_HSM         "IIE School of Hospitality & Service Management <enquiries@iiehsm.com>"
//   MAIL_FROM_NAME_HSM    Display name for the From: header
//   HSM_LOGO_URL          Override URL for the dark HSM logo (letterhead)
//   HSM_BOILER_URL        Override URL for the white IIE boiler banner (footer)
...
// ---------- Brand & contact (https://www.iiehsm.com) ----------
const BRAND = {
  green:    '#76A57C', // primary forest green
  greenDk:  '#5C8762', // darker variant for borders/depth
  mustard:  '#B39432', // golden mustard accent
  charcoal: '#414345', // body / structural text
  dark:     '#2A2C2E', // deepest dark
  grey:     '#7A7C7E', // metadata grey
  neutral:  '#F2F0EA', // cream page background
  border:   '#E2DFD6',
  white:    '#FFFFFF',
  cream:    '#FAF8F1',
};
...
const CONTACT = {
  name:     'The IIE School of Hospitality & Service Management',
  long:     'The IIE School of Hospitality and Service Management',
  short:    'IIEHSM',
  tagline:  'A new standard in industry-aligned service & hospitality education.',
  phone:    '+27 21 881 3792',
  whatsapp: '087 153 4452',
  email:    'enquiries@iiehsm.com',
  enquire:  'portal.emeris.ac.za/enquiry',
  web:      'www.iiehsm.com',
  address:  'IIEHSM \u2014 Rosebank (Johannesburg) & Newlands (Cape Town) campuses',
  legal:    'The IIE School of Hospitality & Service Management (IIEHSM) is an educational brand of The Independent Institute of Education (Pty) Ltd, registered with the DHET as a private higher education institution (reg. no. 2007/HE07/002). IIEHSM is uniting under the Emeris name.',
};

// ---------- Logos (fetched once, embedded as CID) ----------
let LOGO_BUFFER        = null;
let BOILER_BUFFER      = null;
let EMERIS_LOGO_BUFFER = null;

const LOGO_CID         = 'hsm-logo@hsm';
const BOILER_CID       = 'hsm-boiler@hsm';
const EMERIS_LOGO_CID  = 'emeris-white-logo@hsm';
const LOGO_URL         = `cid:${LOGO_CID}`;
const BOILER_URL       = `cid:${BOILER_CID}`;
...
async function loadLogo(req) {
  ...
  LOGO_BUFFER = await fetchFirst([
    process.env.HSM_LOGO_URL,
    base ? `${base}/hsm_logo.png` : null,
    process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}/hsm_logo.png` : null,
  ]);
  return LOGO_BUFFER;
}

async function loadBoiler(req) {
  if (BOILER_BUFFER) return BOILER_BUFFER;
  const base = buildBaseUrl(req);
...
function bodyToHtml(text) {
  ...
  if (/<[a-z][\s\S]*>/i.test(raw)) {
    ...
    html = html.replace(/<a\b([^>]*)>/gi, (m, attrs) => {
      ...
      const baseStyle = `color:${BRAND.mustard};text-decoration:underline;font-weight:600;`;
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      parts[i] = parts[i].replace(
        /(^|[^"'>=])(https?:\/\/[^\s<"']+)/g,
        (m, pre, u) => `${pre}<a href="${u}" target="_blank" rel="noopener noreferrer" style="color:${BRAND.mustard};text-decoration:underline;font-weight:600;">${u}</a>`
      );
    }
    html = parts.join('');
  ...
  const linked = escaped.replace(/\b(https?:\/\/[^\s<]+)/g,
    (u) => `<a href="${u}" target="_blank" rel="noopener noreferrer" style="color:${BRAND.mustard};text-decoration:underline;font-weight:600;">${u}</a>`);
  return linked
    .split(/\n{2,}/)
...
function buildEmailHtml({ subject, body, recipientName }) {
  ...
  const ref = `HSM-${Date.now().toString(36).toUpperCase().slice(-6)}`;
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.neutral};padding:32px 0;">
    <tr><td align="center">
      <table role="presentation" width="660" cellpadding="0" cellspacing="0"
        ...
        <!-- TOP HERO: clean white letterhead with logo left + mustard tagline right -->
        <tr>
          <td bgcolor="${BRAND.white}" style="background-color:${BRAND.white};padding:24px 28px;border-bottom:4px solid ${BRAND.green};">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
              <td valign="middle" align="left" style="width:300px;">
                <img src="${LOGO_URL}" alt="The IIE School of Hospitality &amp; Service Management" height="64"
              ...
              <td valign="middle" align="right" style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
                <div style="color:${BRAND.mustard};font-size:11px;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;line-height:1.2;">
                  Admissions Office
                </div>
                <div style="color:${BRAND.charcoal};font-size:13px;font-weight:600;margin-top:4px;line-height:1.3;">
        ...
        <tr>
          <td style="padding:28px 36px 8px 36px;">
            ...
            <div style="height:3px;width:56px;background:${BRAND.mustard};margin-top:14px;"></div>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:8px 36px 28px 36px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
            ...
            <p style="margin:0;font-size:15px;color:${BRAND.dark};font-weight:700;">
              IIEHSM Admissions &amp; Student Accounts
            </p>
            <p style="margin:2px 0 0 0;font-size:12px;color:${BRAND.mustard};font-style:italic;font-weight:600;">
              ${escapeHtml(CONTACT.tagline)}
            </p>
          </td>
        </tr>
          ...
          <td style="padding:18px 36px 24px 36px;background:${BRAND.cream};font-family:Arial,Helvetica,sans-serif;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
              <td style="font-size:11px;color:${BRAND.charcoal};line-height:1.6;">
                ...
                Email:
                <a href="mailto:${CONTACT.email}" style="color:${BRAND.mustard};text-decoration:none;font-weight:600;">${CONTACT.email}</a>
                &middot; Enquire:
                <a href="https://${CONTACT.enquire}" style="color:${BRAND.mustard};text-decoration:none;font-weight:600;">${CONTACT.enquire}</a>
                <br/>Web:
                <a href="https://${CONTACT.web}" style="color:${BRAND.mustard};text-decoration:none;font-weight:600;">${CONTACT.web}</a>
              </td>
        ...
        <tr>
          <td bgcolor="${BRAND.dark}" valign="middle" style="background-color:${BRAND.dark};padding:14px 32px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
              ...
              <td valign="middle" align="right" style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
                <div style="color:${BRAND.mustard};font-size:11px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;line-height:1.1;">
                  Uniting under Emeris
                </div>
                <div style="color:${BRAND.white};font-size:11px;margin-top:2px;line-height:1.3;">
                  IIEHSM is becoming Emeris &middot; emeris.ac.za
                </div>
              </td>
            </tr></table>
          </td>
      ...
      <div style="font-size:10.5px;color:${BRAND.grey};margin-top:14px;max-width:600px;text-align:center;line-height:1.5;font-family:Arial,Helvetica,sans-serif;">
        This message was sent by The IIE School of Hospitality &amp; Service Management. If you received it in error, please reply to inform us and delete the message.
      </div>
    </td></tr>
  </table>
</body>
...
function extractInteractiveTokens(body) {
  ...
  const html = raw.replace(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi, (m, attrs, inner) => {
    ...
    if (isCta) {
      ...
      const color = bgMatch ? bgMatch[1].trim() : BRAND.mustard;
...
function buildEmailPdfBuffer({ subject, body, recipientName, logo, boiler, emerisLogo }) {
  ...
  const greenDk  = hexToRgb(BRAND.greenDk);
  const mustard  = hexToRgb(BRAND.mustard);
  ...
  // ---- Top hero band: clean white letterhead with logo + mustard tagline ----
  const HERO_H = 86;
  ...
  if (logo && (looksLikePng(logo) || (!looksLikeSvg(logo)))) {
    ...
    doc.setFontSize(16);
    doc.text('IIEHSM', marginX, HERO_H / 2 + 6);
  }

  // Right-aligned mustard tagline block in hero
  doc.setTextColor(mustard[0], mustard[1], mustard[2]);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.text('ADMISSIONS OFFICE', pageW - marginX, HERO_H / 2 - 8, { align: 'right' });
  doc.setTextColor(charcoal[0], charcoal[1], charcoal[2]);
  doc.setFontSize(10);
  doc.text(CONTACT.tagline, pageW - marginX, HERO_H / 2 + 6, { align: 'right' });
  doc.setTextColor(grey[0], grey[1], grey[2]);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.text(CONTACT.web, pageW - marginX, HERO_H / 2 + 20, { align: 'right' });

  // Forest accent stripe under hero
  const accentH = 4;
    ...
    for (const seg of segments) {
      ...
      if (linkMatch) {
        ...
        if (link) {
          ...
          doc.setDrawColor(mustard[0], mustard[1], mustard[2]);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(dark[0], dark[1], dark[2]);
  doc.text('IIEHSM Admissions & Student Accounts', marginX, cursorY);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.text('IIEHSM is becoming Emeris  ·  emeris.ac.za', pageW - marginX, emerisY + emerisH / 2 + 9, { align: 'right' });
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6.5);
    doc.text('IIEHSM is an educational brand of The Independent Institute of Education (Pty) Ltd  ·  DHET reg. 2007/HE07/002',
      pageW / 2, legalY + legalH / 2 + 2, { align: 'center' });

    // Mustard accent at the very bottom
...
function safePdfFilename(subject) {
  const base = String(subject || 'IIEHSM-Communication')
    .replace(/[^A-Za-z0-9 _-]+/g, '')
    .replace(/\s+/g, '_')
    .slice(0, 80) || 'IIEHSM-Communication';
  return `${base}.pdf`;
}
...
export default async function handler(req, res) {
  ...
  if (previewPdf) {
    ...
    try {
      const [logo, boiler, emerisLogo] = await Promise.all([
        loadLogo(req), loadBoiler(req), loadEmerisLogo(req),
      ]);
      const pdfBuffer = buildEmailPdfBuffer({ subject, body, recipientName, logo, boiler, emerisLogo });
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `inline; filename="${safePdfFilename(subject)}"`);
      return res.end(pdfBuffer);
    } catch (err) {
      console.error('IIEHSM PDF preview failed:', err);
      res.statusCode = 500;
      res.setHeader('Content-Type', 'application/json');
  ...
  const FROM_NAME = process.env.MAIL_FROM_NAME_HSM || 'IIE School of Hospitality & Service Management';
  const rawFrom = (process.env.MAIL_FROM_HSM || '').trim();
  let from = '';
  if (rawFrom) {
  ...
  if (logo) {
    ...
    attachments.push({
      filename: isSvg ? 'hsm-logo.svg' : 'hsm-logo.png',
      content: logo,
      cid: LOGO_CID,
  ...
  if (boiler) {
    const isSvg = looksLikeSvg(boiler);
    attachments.push({
      filename: isSvg ? 'iie-boiler.svg' : 'iie-boiler.png',
      content: boiler,
      cid: BOILER_CID,
      contentType: isSvg ? 'image/svg+xml' : (looksLikePng(boiler) ? 'image/png' : 'image/jpeg'),
    });
  }
  if (emerisLogo) {
  ...
  try {
    ...
    console.error('IIEHSM email PDF generation failed:', pdfErr);
  }
