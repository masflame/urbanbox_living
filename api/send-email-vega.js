// Vercel Serverless Function: POST /api/send-email-vega
// Sends a professionally letter-headed HTML email branded for Vega School
// (the IIE creative & brand-leadership school under the Emeris umbrella).
// Uses nodemailer + the SAME SMTP credentials as /api/send-email.
//
// Required env vars (same as send-email.js):
//   SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASS
//   ADMIN_AUTH_TOKEN  shared secret the admin UI sends in x-admin-token
// Optional:
//   MAIL_FROM_VEGA            "Vega School <info@vegaschool.com>"
//                             Falls back to MAIL_FROM, then to SMTP_USER.
//   VEGA_DARK_LOGO_URL        Override URL for the dark wordmark (letterhead)
//   VEGA_WHITE_LOGO_URL       Override URL for the white wordmark (footer)
//   VEGA_BANNER_URL           Override URL for the top hero banner
//   MAIL_FROM_NAME_VEGA       Display name for the From: header
...
// ---------- Brand & contact (https://www.vegaschool.com) ----------
const BRAND = {
  yellow:   '#EAFF00', // primary neon yellow
  purple:   '#230B23', // secondary dark purple / near-black
  lavender: '#DABAEB', // accent lavender
  neutral:  '#F2EEEA', // warm off-white background
  grey:     '#6B5E6B', // muted purple-grey for body text
  border:   '#E4D9E4', // soft lavender border
  white:    '#FFFFFF',
  dark:     '#1A0A1A', // near-black for body copy
};

const CONTACT = {
  name:    'Vega School',
  long:    'Vega School (an IIE brand)',
  short:   'Vega',
  tagline: 'Education that evolves with the world.',
  phone:   '0860 834 200',
  whatsapp:'087 153 4452',
  email:   'info@vegaschool.com',
  enquire: 'www.vegaschool.com/apply',
  web:     'www.vegaschool.com',
  address: 'Vega School, 162 Bram Fischer Drive, Randburg, Johannesburg, 2194',
  legal:   'Vega is an educational brand of The Independent Institute of Education (Pty) Ltd. Registered with the DHET as a private higher education institution under the Higher Education Act, 1997 (reg. no. 2007/HE07/002).',
};
...
async function loadDarkLogo(req) {
  ...
  DARK_LOGO_BUFFER = await fetchFirst([
    process.env.VEGA_DARK_LOGO_URL,
    base ? `${base}/vega_dark_logo.svg` : null,
    process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}/vega_dark_logo.svg` : null,
  ]);
...
async function loadWhiteLogo(req) {
  ...
  WHITE_LOGO_BUFFER = await fetchFirst([
    process.env.VEGA_WHITE_LOGO_URL,
    base ? `${base}/vega_white_footer_logo.png` : null,
    base ? `${base}/vega_white_logo.svg` : null,
...
async function loadBanner(req) {
  ...
  BANNER_BUFFER = await fetchFirst([
    process.env.VEGA_BANNER_URL,
    base ? `${base}/vega-school-emeris-banner.jpg` : null,
    process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}/vega-school-emeris-banner.jpg` : null,
  ]);
...
function bodyToHtml(text) {
  ...
  if (/<[a-z][\s\S]*>/i.test(raw)) {
    ...
    html = html.replace(/<a\b([^>]*)>/gi, (m, attrs) => {
      ...
      const baseStyle = `color:${BRAND.purple};text-decoration:underline;font-weight:600;`;
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      parts[i] = parts[i].replace(
        /(^|[^"'>=])(https?:\/\/[^\s<"']+)/g,
        (m, pre, u) => `${pre}<a href="${u}" target="_blank" rel="noopener noreferrer" style="color:${BRAND.purple};text-decoration:underline;font-weight:600;">${u}</a>`
      );
    }
    html = parts.join('');
    ...
    html = html
      ...
      .replace(/<li\b([^>]*)>/gi, (m, attrs) => {
        const a = (attrs || '').replace(/\sstyle\s*=\s*("[^"]*"|'[^']*')/i, '');
        return `<li${a} style="margin:4px 0;line-height:1.6;color:${BRAND.dark};">`;
      });

    return `<div style="line-height:1.65;color:${BRAND.dark};font-size:15px;">${html}</div>`;
  }
  const escaped = escapeHtml(raw);
  const linked = escaped.replace(/\b(https?:\/\/[^\s<]+)/g,
    (u) => `<a href="${u}" target="_blank" rel="noopener noreferrer" style="color:${BRAND.purple};text-decoration:underline;font-weight:600;">${u}</a>`);
  return linked
    .split(/\n{2,}/)
    .map(p => `<p style="margin:0 0 14px 0;line-height:1.65;color:${BRAND.dark};font-size:15px;">${p.replace(/\n/g,'<br/>')}</p>`)
    .join('');
}
...
function buildEmailHtml({ subject, body, recipientName }) {
  ...
  const ref = `VEG-${Date.now().toString(36).toUpperCase().slice(-6)}`;
...
<body style="margin:0;padding:0;background:${BRAND.neutral};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:${BRAND.dark};">
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="660" cellpadding="0" cellspacing="0"
          ...
          <tr>
            <td style="background:${BRAND.yellow};padding:10px 32px;font-family:Arial,Helvetica,sans-serif;border-bottom:1px solid ${BRAND.purple};">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td valign="middle" style="color:${BRAND.purple};font-size:11px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;">
                    ${today}
                  </td>
                  <td valign="middle" align="right" style="color:${BRAND.purple};font-size:11px;font-weight:700;">
          ...
          <tr>
            <td style="padding:28px 36px 8px 36px;">
              ...
              <div style="font-size:21px;font-weight:700;color:${BRAND.purple};margin-top:6px;line-height:1.3;">
                ${escapeHtml(subject)}
              </div>
              <div style="height:3px;width:56px;background:${BRAND.yellow};margin-top:14px;"></div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:18px 36px 8px 36px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
              <p style="margin:0 0 16px 0;font-size:15px;color:${BRAND.purple};font-weight:700;">
                ${greeting}
              </p>
              ${bodyToHtml(body)}
            </td>
          ...
          <tr>
            <td style="padding:8px 36px 28px 36px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
              <p style="margin:0 0 6px 0;font-size:15px;color:${BRAND.dark};line-height:1.6;">
                Yours sincerely,
              </p>
              <p style="margin:0;font-size:15px;color:${BRAND.purple};font-weight:700;">
                Vega School Admissions &amp; Student Accounts
              </p>
          ...
          <!-- FOOTER -->
          <tr>
            <td style="padding:18px 36px 24px 36px;background:${BRAND.neutral};font-family:Arial,Helvetica,sans-serif;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="font-size:11px;color:${BRAND.grey};line-height:1.6;">
                    <strong style="color:${BRAND.purple};">${CONTACT.name}</strong><br/>
                    ${CONTACT.address}<br/>
                    Tel: ${CONTACT.phone} &middot; Whatsapp: ${CONTACT.whatsapp}<br/>
                    Email:
                    <a href="mailto:${CONTACT.email}" style="color:${BRAND.purple};text-decoration:none;font-weight:600;">${CONTACT.email}</a>
                    &middot; Apply:
                    <a href="https://${CONTACT.enquire}" style="color:${BRAND.purple};text-decoration:none;font-weight:600;">${CONTACT.enquire}</a>
                    <br/>Web:
                    <a href="https://${CONTACT.web}" style="color:${BRAND.purple};text-decoration:none;font-weight:600;">${CONTACT.web}</a>
                  </td>
          ...
          <!-- BOTTOM BRAND BANNER: dark purple band with white logo + tagline -->
          <tr>
            <td bgcolor="${BRAND.purple}" valign="middle"
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td valign="middle" align="right" style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
                    ...
                    <div style="color:${BRAND.lavender};font-size:11px;margin-top:2px;line-height:1.3;font-weight:600;">
                      ${CONTACT.email}
                    </div>
                  </td>
                </tr>
          ...
          <tr>
            <td style="background:${BRAND.dark};padding:12px 32px;font-family:Arial,Helvetica,sans-serif;">
              <div style="color:${BRAND.lavender};font-size:9.5px;line-height:1.5;text-align:center;">
          ...
          <tr>
            <td style="height:6px;background:${BRAND.yellow};line-height:6px;font-size:0;">&nbsp;</td>
          </tr>
        </table>
        ...
        <div style="font-size:10.5px;color:${BRAND.grey};margin-top:14px;max-width:600px;text-align:center;line-height:1.5;font-family:Arial,Helvetica,sans-serif;">
          This message was sent by Vega School. If you received it in error, please reply to inform us and delete the message.
        </div>
      </td>
    </tr>
  </table>
...
function extractInteractiveTokens(body) {
  ...
  const html = raw.replace(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi, (m, attrs, inner) => {
    ...
    if (isCta) {
      ...
      const color = bgMatch ? bgMatch[1].trim() : BRAND.purple;
...
function buildEmailPdfBuffer({ subject, body, recipientName, banner }) {
  ...
  const purple   = hexToRgb(BRAND.purple);
  const yellow   = hexToRgb(BRAND.yellow);
  const lavender = hexToRgb(BRAND.lavender);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(purple[0], purple[1], purple[2]);
  doc.text('Vega School Admissions & Student Accounts', marginX, cursorY);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    // Bottom band: dark purple with yellow accent
    const bannerH = 28;
...
function safePdfFilename(subject) {
  const base = String(subject || 'Vega-Communication')
    .replace(/[^A-Za-z0-9 _-]+/g, '')
    .replace(/\s+/g, '_')
    .slice(0, 80) || 'Vega-Communication';
  return `${base}.pdf`;
}
...
export default async function handler(req, res) {
  ...
  if (previewPdf) {
    ...
    try {
      ...
      console.error('Vega PDF preview failed:', err);
      res.statusCode = 500;
      res.setHeader('Content-Type', 'application/json');
  ...
  const FROM_NAME = process.env.MAIL_FROM_NAME_VEGA || 'Vega School';
  const rawFrom = (process.env.MAIL_FROM_VEGA || '').trim();
  let from = '';
  if (rawFrom) {
  ...
  if (darkLogo) {
    ...
    attachments.push({
      filename: isSvg ? 'vega-dark-logo.svg' : 'vega-dark-logo.png',
      content: darkLogo,
      cid: DARK_LOGO_CID,
  ...
  if (whiteLogo) {
    ...
    attachments.push({
      filename: isSvg ? 'vega-white-logo.svg' : 'vega-white-logo.png',
      content: whiteLogo,
      cid: WHITE_LOGO_CID,
  ...
  if (banner) {
    ...
    attachments.push({
      filename: isPng ? 'vega-banner.png' : 'vega-banner.jpg',
      content: banner,
      cid: BANNER_CID,
  ...
  try {
    ...
    console.error('Vega email PDF generation failed:', pdfErr);
  }
